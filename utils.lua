...
function M.git_repo_name() return vim.fn.system "basename `git rev-parse --show-toplevel` | tr -d '\n'" end

-- Returns the git repo root path
function M.git_repo_root_path()
  local dir = vim.fn.system "git rev-parse --show-toplevel | tr -d '\n'"
  if string.find(dir, "not a git repository") then return nil end
  return dir
end

-- Returns the current workind directory  path
function M.working_dir_path() return vim.fn.getcwd() end

-- Returns the flutter sdk path
function M.flutter_sdk_path() return vim.fn.system "which flutter | tr -d '\n'" end
...
function M.dart_sdk_path() return _flutter_sdk_dart_bin(M.flutter_sdk_path()) end
